plot_diversity <- function(results, Ne_start, Ne_hunted, census_ratio, engine) {
  df <- filter(results, Ne_start == !!Ne_start, Ne_hunted == !!Ne_hunted, census_ratio == !!census_ratio, engine == !!engine)
...
plot_demography <- function(results, Ne_start, Ne_hunted, census_ratio, engine) {
  df <- filter(results, Ne_start == !!Ne_start, Ne_hunted == !!Ne_hunted, census_ratio == !!census_ratio, engine == !!engine)
  model <- df$model[[1]]
...
#     group_by(Ne_start, Ne_hunted, census_ratio, time) %>%
...
#     }) %>%
#     mutate(slope = estimate, Ne_start = paste("Ne start =", Ne_start)) %>%
...
plot_panels <- function(df, Ne_start, Ne_hunted, census_ratio, engine) {
  plot_grid(
    plot_slopes(pi_beluga, engine = engine) + coord_cartesian(ylim = c(-1e-8, 1e-8)),
    plot_grid(
      plot_demography(pi_beluga, Ne_start = Ne_start, Ne_hunted = Ne_hunted, census_ratio = census_ratio, engine),
      plot_diversity(pi_beluga, Ne_start = Ne_start, Ne_hunted = Ne_hunted, census_ratio = census_ratio, engine),
